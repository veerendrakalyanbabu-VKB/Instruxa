...
export function ModelRunner({prompt}:{prompt:string}){
 ...
 const load=useCallback(async()=>{try{const next=await call<Config>("/api/ai/config");setConfig(next);setModel(current=>current||next.defaults[provider])}catch(e){const text=e instanceof Error?e.message:"Sign in to use the model gateway";if(!text.toLowerCase().includes("authentication"))setMessage(text)}},[provider]);
 ...
 function selectProvider(next:Provider){setProvider(next);if(config)setModel(config.defaults[next]);setOutput("");setUsage(null);setMessage("")}
 ...
 <div className="runner-output"><div className="runner-output-head"><span>MODEL RESPONSE</span>{usage&&<small>{usage.inputTokens} input · {usage.outputTokens} output tokens</small>}<Button size="sm" variant="ghost" disabled={!output} onClick={copyOutput}><Clipboard size={14}/>{copied?"Copied":"Copy"}</Button><Button size="sm" variant="ghost" disabled={!output} onClick={download}><Download size={14}/>Export</Button></div>{busy?<div className="runner-empty"><LoaderCircle className="runner-spinner" size={30}/><strong>Gemini is engineering your response</strong><span>Secure execution is in progress. Keep this tab open.</span></div>:output?<Markdown text={output}/>:<div className="runner-empty"><Bot size={30}/><strong>Ready for a real model run</strong><span>Connect a provider key or choose included access when available.</span></div>}{output&&!busy&&<div className="response-footer"><span><CheckCircle2 size={13}/> Secure run complete</span><Button size="sm" variant="ghost" onClick={run}><RotateCcw size={13}/>Run again</Button></div>}</div></div></section>
}