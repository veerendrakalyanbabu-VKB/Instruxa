...
export async function handleAiApi(request:Request,env:AiEnv,user:GatewayUser):Promise<Response>{
 ...
 try{
  ...
  if(url.pathname==="/api/ai/analytics"&&method==="GET"){
   const requestedDays=Number(url.searchParams.get("days")??30),days=[7,30,90].includes(requestedDays)?requestedDays:30,since=`-${days} days`;
   const summary=await env.DB.prepare(`SELECT COUNT(*) AS totalRuns,SUM(CASE WHEN status='succeeded' THEN 1 ELSE 0 END) AS successfulRuns,SUM(CASE WHEN status='failed' THEN 1 ELSE 0 END) AS failedRuns,COALESCE(SUM(input_tokens),0) AS inputTokens,COALESCE(SUM(output_tokens),0) AS outputTokens,COALESCE(ROUND(AVG(CASE WHEN status='succeeded' THEN latency_ms END)),0) AS avgLatencyMs,SUM(CASE WHEN access_mode='byok' THEN 1 ELSE 0 END) AS byokRuns,SUM(CASE WHEN access_mode='included' THEN 1 ELSE 0 END) AS includedRuns FROM usage_events WHERE user_id=? AND created_at>=datetime('now',?)`).bind(user.id,since).first<Record<string,number>>();
   const providers=await env.DB.prepare(`SELECT provider,COUNT(*) AS runs,SUM(CASE WHEN status='succeeded' THEN 1 ELSE 0 END) AS successfulRuns,COALESCE(SUM(input_tokens),0) AS inputTokens,COALESCE(SUM(output_tokens),0) AS outputTokens,COALESCE(ROUND(AVG(CASE WHEN status='succeeded' THEN latency_ms END)),0) AS avgLatencyMs FROM usage_events WHERE user_id=? AND created_at>=datetime('now',?) GROUP BY provider ORDER BY runs DESC`).bind(user.id,since).all();
   const models=await env.DB.prepare(`SELECT provider,model,COUNT(*) AS runs,COALESCE(SUM(input_tokens),0) AS inputTokens,COALESCE(SUM(output_tokens),0) AS outputTokens FROM usage_events WHERE user_id=? AND status='succeeded' AND created_at>=datetime('now',?) GROUP BY provider,model ORDER BY runs DESC LIMIT 12`).bind(user.id,since).all();
   let quality:Array<{provider:string;model:string;overall:number;createdAt:string}>=[],winners:Array<Record<string,unknown>>=[];
   try{
    const rows=await env.DB.prepare(`SELECT provider,model,evaluation_json AS evaluation,created_at AS createdAt FROM ai_runs WHERE user_id=? AND created_at>=datetime('now',?) ORDER BY created_at ASC LIMIT 200`).bind(user.id,since).all();
    quality=(rows.results as Array<{provider:string;model:string;evaluation:string;createdAt:string}>).map(row=>({provider:row.provider,model:row.model,overall:Number(parseEvaluation(row.evaluation)?.overall??0),createdAt:row.createdAt})).filter(row=>row.overall>0);
    try{const result=await env.DB.prepare(`SELECT r.provider,r.model,COUNT(*) AS wins FROM response_winners w JOIN ai_runs r ON r.id=w.run_id WHERE w.user_id=? AND w.created_at>=datetime('now',?) GROUP BY r.provider,r.model ORDER BY wins DESC`).bind(user.id,since).all();winners=result.results as Array<Record<string,unknown>>}catch{}
   }catch(error){if(!String(error).includes("no such table"))throw error}
   const credits=await env.DB.prepare("SELECT balance FROM credit_accounts WHERE user_id=?").bind(user.id).first<{balance:number}>();
   const total=Number(summary?.totalRuns??0),successful=Number(summary?.successfulRuns??0),averageQuality=quality.length?Math.round(quality.reduce((sum,row)=>sum+row.overall,0)/quality.length):0;
   return j({days,summary:{totalRuns:total,successfulRuns:successful,failedRuns:Number(summary?.failedRuns??0),successRate:total?Math.round(successful/total*100):0,inputTokens:Number(summary?.inputTokens??0),outputTokens:Number(summary?.outputTokens??0),avgLatencyMs:Number(summary?.avgLatencyMs??0),byokRuns:Number(summary?.byokRuns??0),includedRuns:Number(summary?.includedRuns??0),averageQuality,credits:credits?.balance??25},providers:providers.results,models:models.results,quality:quality.slice(-30),winners});
  }
